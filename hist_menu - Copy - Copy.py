...
db_config_stock = {
    ...
    'user': 'root',
    'password': 'Mayank#2019',
    'database': 'iia7'  # Update with the correct database name
}
...
def update_historical_data_in_database(analysis_result):
    ...
    try:
        ...
        if existing_data:
            # Symbol already exists, update the existing record in hist_analysis
            cursor.execute("""
            ...
            # Symbol does not exist, insert a new record into hist_analysis
            cursor.execute("""
                INSERT INTO hist_analysis (symbol, cumulative_return, score, company_name)
                VALUES (%s, %s, %s, %s)
            """, (analysis_result['symbol'], analysis_result['cumulative_return'], analysis_result['score'], analysis_result.get('company_name', None)))
        ...
        # Drop the cal_score view
        cursor.execute("DROP VIEW IF EXISTS cal_score")

        # Recreate the cal_score view
        cursor.execute("""
            CREATE 
            VIEW `cal_score` AS
            SELECT `ha`.`symbol` AS `symbol`, `ha`.`cumulative_return` AS `cumulative_return`,
            `ha`.`score` AS `score`, `ha`.`company_name` AS `company_name`, `sa`.`sentiment_score` AS `sentiment_score`
            FROM (`hist_analysis` `ha` LEFT JOIN `sentiment_analysis` `sa`
            ON (`ha`.`company_name` = `sa`.`company_name`))
        """)
        print("cal_score view recreated")
...
def get_top_stocks_to_buy():
    conn = mysql.connector.connect(**db_config_stock)
    cursor = conn.cursor(dictionary=True)

    query = """
        SELECT *
        FROM cal_score
        WHERE sentiment_score > 0 AND score > 0
        ORDER BY sentiment_score + score DESC
        LIMIT 10;
...
def get_top_stocks_to_sell():
    ...
    query = """
        SELECT *
        FROM cal_score
        WHERE sentiment_score < 0 AND historical_score < 0
        ORDER BY sentiment_score + score ASC
        LIMIT 10;
    """
...
while True:
    ...
    print("4. Top Stocks to Buy")
    print("5. Top Stocks to Sell")
    print("6. Exit")
    ...
    if choice == '1':
       ...
       if first_db_found:
        ...
        top_stocks = get_top_stocks_to_buy()

        print("\nTop Stocks to Buy:")
        for stock in top_stocks:
            print(stock)
    elif choice == '5':
        top_stocks1 = get_top_stocks_to_sell()

        print("\nTop Stocks to Sell:")
        for stock in top_stocks1:
            print(stock)
    elif choice == '6':
